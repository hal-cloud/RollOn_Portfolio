...
namespace
{
	constexpr int EFFECT_VERTEX_COUNT = 4;
}

void Effect::Init(Vector3 pos, int width, int height, wchar_t* fileName)
{
	...
	bd.ByteWidth = sizeof(VERTEX_3D) * EFFECT_VERTEX_COUNT;
	...
	m_texture = Texture::Load((wchar_t*)fileName);
	assert(m_texture);
...
void Effect::Draw(void)
{
	...
	Renderer::GetDeviceContext()->Draw(EFFECT_VERTEX_COUNT, 0);
...
void Effect::SetVertexColor(int index, const XMFLOAT4& color)
{
	if (index < 0 || index >= EFFECT_VERTEX_COUNT)
	{
		return;
	}
...
void Effect::SetVertexColors(const std::array<XMFLOAT4, EFFECT_VERTEX_COUNT>& colors)
{
	for (int i = 0; i < EFFECT_VERTEX_COUNT; ++i)
	{
...
void Effect::UpdateVertexBuffer()
{
	if (!m_vertexBuffer)
	{
		return;
...
void Effect::SetPixelShader(const char* pixelShaderCsoPath)
{
